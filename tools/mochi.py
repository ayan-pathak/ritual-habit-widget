...
# Abutting shapes each antialias their own edge, so the background shows
# through the boundary as a hairline. Growing every shape by half a pixel of
# stroke in its own colour covers it. Everywhere else that stroke is one
# device pixel, set per draw; an adaptive icon is a drawable and cannot do
# that, so its width is baked for the 48-192px an icon is actually drawn at.
ICON_SEAM = 1.6
...
def render_vector(mood, fn, view, size, head=()):
    ...
    seam = ' android:strokeWidth="%s" android:strokeLineJoin="round"' % fmt(ICON_SEAM)
    for d, fill, alpha in paths:
        gid = re.match(r'url\(#(\w+)\)', fill)
        if gid is None:
            line = '    <path android:fillColor="%s" android:strokeColor="%s"%s' % (
                argb(fill), argb(fill), seam)
            line += '\n        android:pathData="%s"' % d
            if alpha < 1.0:
                line += '\n        android:fillAlpha="%s" android:strokeAlpha="%s"' % (
                    fmt(alpha), fmt(alpha))
            out.append(line + '/>')
            continue
...
ROW_FORMAT = (
    ...
    'already registered, in a %s x %s viewport.',
)


def rows_for(mood):
    """The wire format both platforms read. One encoder, so they cannot
...
def camel(mood):
    return ''.join(p.capitalize() if i else p for i, p in enumerate(mood.split('_')))
...
def swift():
    ...
    """
    blocks = [(m, rows_for(m)) for m in MOODS]

    out = ['import CoreGraphics', '',
           '// Generated by tools/mochi.py from art/mochi/*.svg. Do not edit.',
           '//',
           ] + ['// ' + line % (fmt(VIEW_W), fmt(VIEW_H)) if '%s' in line else '// ' + line
                for line in ROW_FORMAT] + [
           'enum MochiArt {', '',
...
def kotlin_art():
    """The same rows for Android, which replays them rather than handing a
    VectorDrawable to the Canvas.

    A VectorDrawable cannot stroke in device pixels, and a device-pixel stroke
    is the only thing that closes the seams between abutting shapes at every
    size. Replaying the paths also puts both platforms on one drawing model.
    """
    out = ['package com.ayan.ritual.render', '',
           '// Generated by tools/mochi.py from art/mochi/*.svg. Do not edit.',
           '//'] + ['// ' + (line % (fmt(VIEW_W), fmt(VIEW_H)) if '%s' in line else line)
                    for line in ROW_FORMAT] + [
           'object MochiArt {', '',
           '    const val VIEW_W = %sf' % fmt(VIEW_W),
           '    const val VIEW_H = %sf' % fmt(VIEW_H), '']
    for mood in MOODS:
        out.append('    const val %s = """\n%s\n"""\n' % (camel(mood), rows_for(mood)))
    out.append('}')
    return '\n'.join(out) + '\n'
...
def targets():
    return {
        os.path.join(ROOT, 'app/src/main/res/drawable/ic_launcher_foreground.xml'):
            launcher_foreground(),
        os.path.join(ROOT, 'app/src/main/java/com/ayan/ritual/render/MochiArt.kt'):
            kotlin_art(),
        os.path.join(ROOT, 'ios/Shared/MochiArt.swift'): swift(),
    }
